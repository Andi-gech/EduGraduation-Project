...
import UseFechEnrollTransaction from "../../hooks/UseFechEnrollTransaction";

export default function EnrollmentTransactions() {
  const { data, isLoading } = UseFechEnrollTransaction();
  ...
  const rows = useMemo(() => {
    const mappedRows = data?.data?.map((item, index) => ({
      ...
      fullname: `${item.studentId?.firstName} ${item.studentId?.lastName}`,
      Status: item.status,
    }));
  ...
  const columns = [
    ...
    {
     field:"yearLevel",headerName:"YearLevel",width:50
    },
    {
        field:"semister",headerName:"semister",width:50
    },

    { field: "fullname", headerName: "Full Name", width: 160 },
   
    { field: "_id", headerName: "Txn id", width: 160 },
